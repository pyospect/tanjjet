...
final class SupabaseService {
    ...
    func disconnectCouple() async throws {
        do {
            try await disconnectCoupleWithFunction()
            return
        } catch {
            AppLogger.error("Disconnect Edge Function failed, falling back to RPC: \(error.localizedDescription)")
        }

        let didDisconnect = try await disconnectCoupleWithRPC()
        if !didDisconnect {
            try await updateProfile(ProfileUpdate(clearCoupleId: true))
        }
    }

    private func disconnectCoupleWithFunction() async throws {
        let accessToken = try await client.auth.session.accessToken
        var request = URLRequest(url: supabaseURL.appendingPathComponent("functions/v1/disconnect-couple"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue(supabaseKey, forHTTPHeaderField: "apikey")
        request.setValue("Bearer \(accessToken)", forHTTPHeaderField: "Authorization")
        ...
        guard (200..<300).contains(httpResponse.statusCode) else {
            let message = Self.functionErrorMessage(from: data) ?? "연결 해제 요청에 실패했습니다"
            throw SupabaseError.server(message)
        }
    }

    private func disconnectCoupleWithRPC() async throws -> Bool {
        guard let userId = await currentUserId else {
            throw SupabaseError.notAuthenticated
        }
